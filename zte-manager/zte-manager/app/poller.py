...
from app.config import OltConfig, OidProfile
...
async def poll_all(
    olts: list[OltConfig], oid_profiles: dict[str, OidProfile], session_factory
) -> dict[str, int]:
    """
    Poll semua OLT yang diberikan (biasanya diambil fresh dari DB tiap
    kali dipanggil, supaya OLT yang baru ditambah lewat UI otomatis ikut
    kepoll di siklus berikutnya tanpa perlu restart server).
    """
    results: dict[str, int] = {}
    for olt in olts:
        profile = oid_profiles.get(olt.profile)
        if profile is None:
